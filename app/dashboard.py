...
def build_dashboard(
    ...
    with open(input_file) as f:
        for row in csv.DictReader(f):
            ...
            # Monthly charts and summaries must use only monthly records.
            # Daily records are kept separately for the recent daily view and
            # must never be added to the monthly totals.
            if row["period"] == "month":
                key = row["date"][:7]
